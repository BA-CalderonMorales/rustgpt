...
impl Layer for FeedForward {
    ...
    fn backward(&mut self, grads: &Array2<f32>, lr: f32) -> Array2<f32> {
        // Unwrap the cached forward state.
        ...
        // Gradients for the input projection (W1, b1).
        let grad_w1 = input.t().dot(&grad_hidden_pre_activation);
        let grad_b1 = grad_hidden_pre_activation
            .sum_axis(Axis(0))
            .insert_axis(Axis(0));

        // Gradient back to the input, plus the residual copy.
        let grad_input = grad_hidden_pre_activation.dot(&self.w1.t()) + grads;

        // Update every parameter through its optimizer.
    ...
    fn forward(&mut self, input: &Array2<f32>) -> Array2<f32> {
        // Two-layer MLP: hidden projection, ReLU, then output projection.
