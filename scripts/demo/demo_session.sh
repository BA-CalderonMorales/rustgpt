#!/usr/bin/env bash
# The v0.0.6 showcase session, recorded with `script` and rendered by agg.
...
echo
echo "=== 1. the release checkpoint: v0.0.6, version contract pinned by tests ==="
sleep 1
...
echo
echo "=== 3. the domain-labeled trace: every generated token carries its domain ==="
sleep 1
cp models/watercycle-latest.bin /tmp/opencode/trace-demo.bin
printf 'yo!\nhello!\nwater?\nexit\n' | ./target/release/llm --trace --model /tmp/opencode/trace-demo.bin --seed 42 2>&1 | tail -30
sleep 4

echo
echo "=== 4. micro arena truth table: train both phases, score the held-out prompts ==="
sleep 1
...
echo
echo "=== 5. the laptop lane: 7.3M params trained on real story text ==="
sleep 1
...
echo
echo "=== checkpoint saved. the collapse gate is the number 0.0.6 attacks. ==="
sleep 3
